...
module.exports = function(Salesessions) {
    Salesessions.beforeRemote('**', function(ctx, user, next) {
        if (!ctx.args.options) return next();
        if (!ctx.args.options.accessToken) return next();

        Salesessions.app.models.company_user.findOne({//Get user in company
			where : { user_id : ctx.args.options.accessToken.userId }
		},function(err, data){
            if (err) return next(err);            
			Salesessions.app.models.connections.findOne({//Get connection
